...
const Post = ({data}) => {
  ...
  return (
    <Card fluid>
      ...
      <Card.Content>
        {data.user_id === user?.id && <Button floated='right'>
          <Dropdown floated='right' icon='edit'>
            <Dropdown.Menu>
              <Dropdown.Item text='Delete' onClick={() => deletePost(data.id)} />
              <Dropdown.Item text='Edit'>
                <Update setNewPost={setNewPost} id={data.id} title={data.title} body={data.content} />
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
        </Button>}
        ...
        <Card.Content extra style={{display: 'flex', justifyContents: 'space-between'}}>
          <Card.Description style={{marginRight:'4px'}}>{likes.length}</Card.Description>
          <Card.Description style={{marginRight:'4px'}} onClick={() => likePost()}>{isLiked ? 'Un-Like' : 'Like'}</Card.Description>
          <Card.Description onClick={() => getComments()}>Comments</Card.Description>
        </Card.Content>
