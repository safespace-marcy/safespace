            ...
            <Comment.Group size='large' id='chatBox' style={{ marginTop: '25px', overflow: 'auto', height: '400px', border: '3px solid #a7abaf', width: '100%', padding: '25px' }}>
              {
                chatLog.map((message, index) => {
                  if (message.socketId === yourId) {
                    return (
                      <Message key={index} isPrivate message={message.body} author={message.username} sentAt={message.sentAt} sprite={message.sprite} seed={message.seed} yourId={yourId} contact={contact} />
                    )
                  }
                  return (
